...
const PostsSection = () => {
    ...
    return (
        <div>
            {/* Checks if userFeed is empty */}
            {userFeed.length === 0 ? (
                <p className="text-center text-gray-500 mt-8 font-semibold">
                    No posts available.
                </p>
            ) : (
                <div className="grid grid-cols-2 gap-[0.75rem] mt-[0.68rem]">
                    {userFeed.map((i) => (
                        <div key={i.id} className="relative w-full h-[15rem] rounded-[0.75rem] bg-[#ffffff]">
